...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  const hasGoogleAIStudioKey = !!env.GOOGLE_AI_STUDIO_API_KEY;

  if (!hasCloudflareGateway && !hasLegacyGateway && !hasAnthropicKey && !hasOpenAIKey && !hasGoogleAIStudioKey) {
    missing.push(
      'ANTHROPIC_API_KEY, OPENAI_API_KEY, GOOGLE_AI_STUDIO_API_KEY, or CLOUDFLARE_AI_GATEWAY_API_KEY + CF_AI_GATEWAY_ACCOUNT_ID + CF_AI_GATEWAY_GATEWAY_ID',
    );
  }
...
app.all('*', async (c) => {
  ...
  try {
    ...
    if (!c.env.ANTHROPIC_API_KEY && !c.env.OPENAI_API_KEY && !c.env.GOOGLE_AI_STUDIO_API_KEY) {
      hint = 'No AI provider key is set. Run: wrangler secret put GOOGLE_AI_STUDIO_API_KEY';
